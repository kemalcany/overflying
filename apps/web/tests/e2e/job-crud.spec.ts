...
test.describe('Job CRUD Operations', () => {
  ...
  test('complete CRUD flow: create, edit, and delete a job', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'E2E Test Job'}).first(),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('heading', {name: 'Updated E2E Job'}).first(),
    ).toBeVisible();
    ...
    const updatedJobCard = page
      .locator('article')
      .filter({hasText: 'Updated E2E Job'});
    await updatedJobCard.getByRole('button', {name: /delete/i}).click();

    // Confirmation dialog should open
    await expect(
      page.getByRole('heading', {name: /delete job/i}),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('heading', {name: 'Updated E2E Job'}),
    ).not.toBeVisible();
  });
  ...
  test('can create multiple jobs', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'Job 1'}).first(),
    ).toBeVisible();
    await expect(
      page.getByRole('heading', {name: 'Job 2'}).first(),
    ).toBeVisible();
  });
  ...
  test('can cancel job creation', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'Cancelled Job'}),
    ).not.toBeVisible();
  });
  ...
  test('can cancel job edit', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'Original Job'}).first(),
    ).toBeVisible();
    await expect(
      page.getByRole('heading', {name: 'Changed Job'}),
    ).not.toBeVisible();
  });
  ...
  test('can cancel job deletion', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'Job to Keep'}).first(),
    ).toBeVisible();
  });
  ...
  test('displays job metadata correctly', async ({page}) => {
    ...
    await expect(
      page.getByRole('heading', {name: 'Metadata Job'}).first(),
    ).toBeVisible();
  ...
  test('shows empty state when no jobs exist', async ({page}) => {
    ...
    if (count === 0) {
      ...
      const hasEmptyMessage =
        (await page.getByText(/no jobs/i).count()) > 0 ||
        (await page.getByText(/create your first/i).count()) > 0 ||
